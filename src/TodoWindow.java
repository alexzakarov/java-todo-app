...
public class TodoWindow {
    ...
    public TodoWindow(int width, int height, TodoRepository todoRepository, Boolean existTodoScreen1) {
    ...
    public void saveButton(int x, int y, TodoWindow todoWindow, MainWindow mainWindow) {
        JButton b = new JButton("Save");
        b.setLocation(x, y);
        b.setSize(150, 30);
        this.saveEventListener(b, todoWindow, mainWindow);
        this.frame.add(b);
    }


    public void updateButton(int x, int y, TodoWindow todoWindow, MainWindow mainWindow, int todoId) {
        JButton b = new JButton("Update");
        b.setLocation(x, y);
        b.setSize(150, 30);
        this.updateEventListener(b, todoWindow, mainWindow, todoId);
        this.frame.add(b);
    }

    public void addText(int x, int y, String text,TodoWindow todoWindow) {
    ...
    public void addTextEventListener(JButton button, TodoWindow todoWindow) {
        button.addActionListener(ActionListener -> {
           if(this.textSize < 15) {
               todoWindow.addTextArea(100,this.textSize*20+80,200,20, this.textSize);
               todoWindow.show();
               this.textSize++;
           }
        });
    ...
    public void saveEventListener(JButton button, TodoWindow todoWindow, MainWindow mainWindow) {
        button.addActionListener(ActionListener -> {
            int size = this.textAreas.size();
    ...
    public void updateEventListener(JButton button, TodoWindow todoWindow, MainWindow mainWindow, int todoId) {
        button.addActionListener(ActionListener -> {
            int size = this.textAreas.size();
            TodoEntity todo = new TodoEntity();
            todo.subject = this.textAreas.get(0).getText();
        ...
        frame.addWindowListener(new java.awt.event.WindowAdapter() {
            ...
            public void windowClosing(java.awt.event.WindowEvent windowEvent) {
                if (JOptionPane.showConfirmDialog(frame,
                    ...
                    String subject = textAreas.get(0).getText();
                    List<String> todos = new ArrayList<>();

                    for(int i = 1; i < textAreas.size(); i++){
                        todos.add(textAreas.get(i).getText());
                    }
                    todoWindow.removeTextAreas();
                    todoWindow.textSize = 1;
                    todoWindow.hide();
